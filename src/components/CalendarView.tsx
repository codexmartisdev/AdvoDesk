...
const isOpenStatus = (status?: ScheduledEvent['status']) => status === 'Pendente' || status === 'Remarcado';
...
export const CalendarView: React.FC<CalendarViewProps> = ({
  ...
  const summary = useMemo(() => {
    const openEvents = events.filter((event) => isOpenStatus(event.status));
    return {
  ...
  const filteredEvents = useMemo(() => {
    ...
    return [...events]
      .filter((event) => {
        ...
        const days = getDaysRemaining(event.fullDate);
        const open = isOpenStatus(event.status);
        ...
        if (operationalFilter === 'urgentes') {
          matchesOperational = open && (event.priorityLevel === 'urgente' || days < 3);
        }
  ...
  const handleSaveEvent = async (event: React.FormEvent) => {
    ...
    if (!formTitle.trim()) {
      setFormError('Informe um título para o compromisso.');
      return;
    }
    if (!formFullDate) {
      setFormError('Informe a data do compromisso.');
      return;
    }

    const linkedCase = formCaseId ? activeCases.find((item) => item.id === formCaseId) : undefined;
    if (formCaseId && !linkedCase) {
      setFormError('O caso vinculado não está mais disponível. Atualize a seleção.');
      return;
    }

    const savingOpenEvent = isOpenStatus(formStatus);
    if (savingOpenEvent) {
      const duplicate = events.some((existing) => {
        if (existing.id === editingEvent?.id || !isOpenStatus(existing.status)) return false;
      ...
      if (duplicate) {
        setFormError('Já existe um compromisso em aberto com o mesmo caso, tipo e data.');
        return;
      }
    }
    ...
    if (formSyncWithGoogle && googleAccessToken) {
      try {
        ...
        setGoogleSyncMessage({
          text: `Salvo no AdvoDesk, mas o Google Agenda não sincronizou: ${error?.message || 'erro desconhecido'}`,
          type: 'error',
        });
      }
    }
    ...
    <main className="md:ml-64 pt-16 md:pt-8 pb-12 px-4 sm:px-6 md:px-8 min-h-screen relative z-10 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between pb-3 border-b border-slate-200/60 gap-4">
        <div>
          ...
          <p className="text-slate-600 text-sm">Controle operacional de prazos, audiências, perícias e compromissos vinculados ao contexto real dos casos.</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <button type="button" onClick={() => applyOperationalFilter('vencidos')} className={`text-left rounded-2xl border p-3.5 transition-all ${operationalFilter === 'vencidos' ? 'bg-red-100 border-red-300' : 'bg-red-50/90 border-red-200 hover:bg-red-100/70'}`}>
          <div className="text-[10px] uppercase tracking-wider font-extrabold text-red-700">Vencidos</div>
          <div className="text-2xl font-black text-red-950 mt-1">{summary.overdue}</div>
        </button>
        <button type="button" onClick={() => applyOperationalFilter('hoje')} className={`text-left rounded-2xl border p-3.5 transition-all ${operationalFilter === 'hoje' ? 'bg-amber-100 border-amber-300' : 'bg-amber-50/90 border-amber-200 hover:bg-amber-100/70'}`}>
          <div className="text-[10px] uppercase tracking-wider font-extrabold text-amber-800">Hoje</div>
          <div className="text-2xl font-black text-amber-950 mt-1">{summary.today}</div>
        </button>
        <button type="button" onClick={() => applyOperationalFilter('7dias')} className={`text-left rounded-2xl border p-3.5 transition-all ${operationalFilter === '7dias' ? 'bg-blue-100 border-blue-300' : 'bg-blue-50/90 border-blue-200 hover:bg-blue-100/70'}`}>
          <div className="text-[10px] uppercase tracking-wider font-extrabold text-blue-900">Próximos 7 dias</div>
          <div className="text-2xl font-black text-blue-950 mt-1">{summary.nextSevenDays}</div>
        </button>
        <button type="button" onClick={() => applyOperationalFilter('todos')} className={`text-left rounded-2xl border p-3.5 transition-all ${operationalFilter === 'todos' ? 'bg-slate-200 border-slate-300' : 'bg-slate-100/90 border-slate-200 hover:bg-slate-200/70'}`}>
          <div className="text-[10px] uppercase tracking-wider font-extrabold text-slate-600">Em aberto</div>
          <div className="text-2xl font-black text-slate-900 mt-1">{summary.open}</div>
        </button>
      </div>
      ...
      <div className="bg-white rounded-2xl p-3 border border-slate-200 shadow-2xs flex flex-col xl:flex-row xl:items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-2 overflow-x-auto pb-1 xl:pb-0">
          ...
          {EVENT_TYPES.map((type) => (
            <button key={type} onClick={() => setFilterType(type)} className={`px-3 py-1.5 rounded-lg font-bold whitespace-nowrap ${filterType === type ? 'bg-blue-900 text-white' : 'bg-slate-100 text-slate-600'}`}>{type}</button>
          ))}
        </div>
      ...
      {googleSyncMessage && (
        <div className={`p-3.5 rounded-2xl border text-xs font-bold flex items-center justify-between ${googleSyncMessage.type === 'success' ? 'bg-emerald-50 text-emerald-900 border-emerald-200' : 'bg-red-50 text-red-900 border-red-200'}`}>
          ...
          <button type="button" onClick={() => setGoogleSyncMessage(null)}><span className="material-symbols-outlined text-sm">close</span></button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-8 space-y-3">
          {selectedDayFilter && (
            <div className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 flex items-center justify-between text-xs text-blue-950">
              <span className="font-bold">Exibindo {formatToPtBR(selectedDayFilter)}</span>
              <button type="button" onClick={() => setSelectedDayFilter(null)} className="font-bold hover:underline">Mostrar todas as datas</button>
            </div>
          )}
              ...
              <article key={event.id} onClick={(mouseEvent) => handleOpenEditModal(event, mouseEvent)} className={`rounded-2xl p-5 bg-white border border-slate-200 shadow-2xs hover:shadow-md transition-all cursor-pointer group relative overflow-hidden ${event.status === 'Concluído' ? 'opacity-80' : ''}`}>
                ...
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 pl-1">
                  <div className="flex items-start gap-4 min-w-0">
                    <div className="w-16 h-16 rounded-xl border border-slate-200 bg-slate-50 flex flex-col items-center justify-center shrink-0">
                      <span className="text-[10px] uppercase font-bold text-slate-500">{event.dateStr?.split(' ')[1] || formatDateBadge(event.fullDate).split(' ')[1]}</span>
                      <span className="text-xl font-black text-slate-900">{event.dateStr?.split(' ')[0] || formatDateBadge(event.fullDate).split(' ')[0]}</span>
                    </div>
                    <div className="space-y-1.5 min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {linkedCase && <span className="px-2 py-0.5 rounded-full bg-slate-100 border border-slate-200 text-[10px] font-bold text-slate-600">{linkedCase.caseNumber}</span>}
                      </div>
                      <h3 className="font-bold text-slate-900 group-hover:text-blue-900 transition-colors">{event.title}</h3>
                      {(event.clientName || event.processNumber || event.benefitType) && (
                        <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-slate-600">
                          {event.clientName && <span className="font-bold text-slate-800">{event.clientName}</span>}
                          {event.benefitType && <span>{event.benefitType}</span>}
                          {event.processNumber && <span className="font-mono text-[11px]">{event.processNumber}</span>}
                        </div>
                      )}
                      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500">
                        <span className="font-semibold text-slate-700">{formatToPtBR(event.fullDate)}{event.time ? ` às ${event.time}` : ''}</span>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0 self-end sm:self-center" onClick={(mouseEvent) => mouseEvent.stopPropagation()}>
                    {event.status === 'Concluído' ? (
                      <button type="button" onClick={(mouseEvent) => handleQuickStatusChange(event, 'Pendente', mouseEvent)} className="px-2.5 py-1.5 rounded-xl bg-slate-100 text-slate-700 border border-slate-300 text-xs font-bold">Reabrir</button>
                    ) : (
                      <>
                        <button type="button" onClick={(mouseEvent) => handleQuickStatusChange(event, 'Concluído', mouseEvent)} className="px-2.5 py-1.5 rounded-xl bg-emerald-50 text-emerald-800 border border-emerald-200 text-xs font-bold">Concluir</button>
                        <button type="button" onClick={(mouseEvent) => handleOpenReschedule(event, mouseEvent)} className="px-2.5 py-1.5 rounded-xl bg-purple-50 text-purple-800 border border-purple-200 text-xs font-bold">Remarcar</button>
                      </>
                    )}
          ...
          {filteredEvents.length === 0 && (
            <div className="bg-white rounded-2xl p-12 text-center text-slate-500 border border-slate-200 shadow-2xs space-y-3">
              ...
              <p className="font-bold text-slate-800">Nenhum compromisso encontrado.</p>
              <p className="text-xs">Ajuste os filtros ou cadastre um novo evento.</p>
              <button type="button" onClick={handleOpenCreateModal} className="glass-btn-primary px-4 py-2 rounded-xl text-white font-bold text-xs">Novo evento</button>
            </div>
          )}
        </div>
        ...
        <aside className="lg:col-span-4 space-y-4">
          <div className="rounded-2xl p-5 bg-white border border-slate-200 shadow-2xs">
            <div className="flex items-center justify-between gap-2 mb-4">
              <button type="button" title="Mês anterior" onClick={() => shiftMonth(-1)} className="p-2 rounded-xl hover:bg-slate-100 text-slate-600"><span className="material-symbols-outlined">chevron_left</span></button>
              <div className="text-center">
                <h3 className="font-extrabold text-slate-900 text-sm">{monthLabel}</h3>
                <button type="button" onClick={goToCurrentMonth} className="text-[10px] font-bold text-blue-900 hover:underline">Ir para hoje</button>
              </div>
              <button type="button" title="Próximo mês" onClick={() => shiftMonth(1)} className="p-2 rounded-xl hover:bg-slate-100 text-slate-600"><span className="material-symbols-outlined">chevron_right</span></button>
            </div>
            ...
            <div className="grid grid-cols-7 gap-1 text-center">
              {calendarDays.map((isoDate, index) => {
                ...
                const openDayEvents = dayEvents.filter((event) => isOpenStatus(event.status));
                ...
                const hasAttention = openDayEvents.some((event) => {
                  const days = getDaysRemaining(event.fullDate);
                  return days >= 3 && days < 7;
                });

                return (
                  <button key={isoDate} type="button" onClick={() => setSelectedDayFilter((current) => current === isoDate ? null : isoDate)} className={`h-9 rounded-xl relative text-xs font-bold transition-all ${isSelected ? 'bg-blue-900 text-white shadow-xs' : isToday ? 'bg-blue-100 text-blue-950 border border-blue-300' : 'hover:bg-slate-100 text-slate-800'}`}>
                    {dayNumber}
                    {dayEvents.length > 0 && (
                      <span className={`absolute bottom-1 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full ${hasOverdueOrUrgent ? 'bg-red-600' : hasAttention ? 'bg-amber-500' : openDayEvents.length > 0 ? 'bg-emerald-500' : 'bg-slate-400'}`} />
                    )}
                  </button>
                );
              })}
            </div>
            <div className="mt-4 pt-3 border-t border-slate-100 text-[10px] text-slate-500 grid grid-cols-2 gap-2">
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-600" /> Urgente/vencido</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-500" /> Atenção</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-emerald-500" /> Em aberto</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-slate-400" /> Histórico</span>
            </div>
          </div>
          ...
          <div className="rounded-2xl p-4 bg-white border border-slate-200 shadow-2xs">
            <div className="flex items-center justify-between gap-3">
              <div>
                <div className="text-xs font-bold text-slate-900">Google Agenda</div>
                <div className="text-[10px] text-slate-500 mt-0.5">{googleUser ? googleUser.email : 'Não conectado'}</div>
              </div>
              {googleUser ? (
                <button type="button" onClick={() => void handleDisconnectGoogle()} className="px-3 py-2 rounded-xl bg-slate-100 text-slate-700 text-xs font-bold">Desconectar</button>
              ) : (
                <button type="button" disabled={isLoggingInGoogle} onClick={() => void handleConnectGoogle()} className="px-3 py-2 rounded-xl bg-blue-900 text-white text-xs font-bold disabled:opacity-50">{isLoggingInGoogle ? 'Conectando...' : 'Conectar'}</button>
              )}
            </div>
          </div>
        </aside>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-xs overflow-y-auto">
          <div className="bg-white w-full max-w-2xl rounded-3xl border border-slate-200 shadow-2xl relative my-auto max-h-[92vh] flex flex-col overflow-hidden text-left">
            <div className="p-6 border-b border-slate-200 bg-slate-50/70 flex items-start justify-between gap-4">
              <div>
                <h3 className="text-lg font-extrabold text-slate-900">{editingEvent ? 'Editar evento / prazo' : 'Novo evento / prazo'}</h3>
                <p className="text-xs text-slate-500 mt-1">Vincule ao caso para reutilizar cliente, processo e área sem redigitação.</p>
              </div>
              <button type="button" onClick={() => setModalOpen(false)} className="p-2 rounded-xl hover:bg-slate-200 text-slate-500"><span className="material-symbols-outlined">close</span></button>
            </div>
            ...
            <form id="agenda-event-form" onSubmit={handleSaveEvent} className="p-6 space-y-5 text-xs overflow-y-auto">
              ...
              <section className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
                ...
                <div>
                  <label className="block font-bold text-slate-700 mb-1">Caso / processo vinculado</label>
                  <select value={formCaseId} onChange={(event) => handleCaseSelectChange(event.target.value)} className="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 font-semibold">
                    <option value="">Sem caso vinculado</option>
                    {activeCases.map((legalCase) => <option key={legalCase.id} value={legalCase.id}>{legalCase.clientName} — {legalCase.title} ({legalCase.processNumber || legalCase.caseNumber})</option>)}
                  </select>
                </div>

                {!formCaseId && (
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">Cliente relacionado <span className="font-normal text-slate-400">(opcional)</span></label>
                    <select value={formClientId} onChange={(event) => handleClientSelectChange(event.target.value)} className="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 font-semibold">
                      <option value="">Sem cliente relacionado</option>
                      {availableClients.map((client) => <option key={client.id} value={client.id}>{client.name} — CPF {client.cpf}</option>)}
                    </select>
                  </div>
                )}
                ...
                {(formCaseId || formClientId || formClientName) && (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div><label className="block font-bold text-slate-700 mb-1">Cliente</label><input value={formClientName} onChange={(event) => setFormClientName(event.target.value)} readOnly={Boolean(formCaseId || formClientId)} className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2 read-only:bg-slate-100" /></div>
                    <div><label className="block font-bold text-slate-700 mb-1">Área / ação</label><input value={formBenefitType} onChange={(event) => setFormBenefitType(event.target.value)} readOnly={Boolean(formCaseId)} className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2 read-only:bg-slate-100" /></div>
                    <div><label className="block font-bold text-slate-700 mb-1">Processo / protocolo</label><input value={formProcessNumber} onChange={(event) => setFormProcessNumber(event.target.value)} readOnly={Boolean(formCaseId)} className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2 read-only:bg-slate-100" /></div>
                  </div>
                )}
              ...
              <section className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div><label className="block font-bold text-slate-700 mb-1">Tipo</label><select value={formEventType} onChange={(event) => handleEventTypeChange(event.target.value)} className="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 font-semibold">{EVENT_TYPES.map((type) => <option key={type}>{type}</option>)}</select></div>
                  <div><label className="block font-bold text-slate-700 mb-1">Título *</label><input required value={formTitle} onChange={(event) => setFormTitle(event.target.value)} className="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5" placeholder="Ex: Audiência de conciliação" /></div>
                  <div><label className="block font-bold text-slate-700 mb-1">Data *</label><input required type="date" value={formFullDate} onChange={(event) => setFormFullDate(event.target.value)} className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2.5" /></div>
                  <div><label className="block font-bold text-slate-700 mb-1">Horário</label><input type="time" value={formTime} onChange={(event) => setFormTime(event.target.value)} className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2.5" /></div>
                  <div className="md:col-span-2"><label className="block font-bold text-slate-700 mb-1">Local / link</label><input value={formLocation} onChange={(event) => setFormLocation(event.target.value)} className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2.5" placeholder="Fórum, agência, escritório ou link da videoconferência" /></div>
                </div>
              </section>
              ...
              <section className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div><label className="block font-bold text-slate-700 mb-1">Status</label><select value={formStatus} onChange={(event) => setFormStatus(event.target.value as ScheduledEvent['status'])} className="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 font-semibold"><option>Pendente</option><option>Remarcado</option><option>Concluído</option><option>Perdido</option></select></div>
                  <div><label className="block font-bold text-slate-700 mb-1">Prioridade visual</label><select value={formPriorityLevel} onChange={(event) => setFormPriorityLevel(event.target.value as PriorityFormValue)} className="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 font-semibold"><option value="auto">Automática pela data</option><option value="urgente">Urgente</option><option value="atencao">Atenção</option><option value="normal">Normal</option></select></div>
                </div>
                ...
                <div><label className="block font-bold text-slate-700 mb-1">Observação rápida</label><textarea rows={3} value={formNotes} onChange={(event) => setFormNotes(event.target.value)} className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2 resize-none" placeholder="Informação necessária para executar o compromisso." /></div>
              </section>

              <section className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
                <h4 className="font-extrabold text-blue-900 uppercase tracking-wider text-[10px]">Lembrete</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div><label className="block font-bold text-slate-700 mb-1">Antecedência</label><select value={formReminderOption} onChange={(event) => setFormReminderOption(event.target.value as ReminderFormValue)} className="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 font-semibold"><option value="1_hour">1 hora antes</option><option value="1_day">1 dia antes</option><option value="2_days">2 dias antes</option><option value="custom">Horário personalizado no dia</option><option value="at_time">Na hora</option></select></div>
                  {formReminderOption === 'custom' && <div><label className="block font-bold text-slate-700 mb-1">Horário do alerta</label><input type="time" value={formCustomReminderTime} onChange={(event) => setFormCustomReminderTime(event.target.value)} className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2.5" /></div>}
                </div>
                <label className="flex items-center gap-2 text-xs font-semibold text-slate-700"><input type="checkbox" checked={formSyncWithGoogle} onChange={(event) => setFormSyncWithGoogle(event.target.checked)} /> Sincronizar no Google Agenda quando conectado</label>
              </section>
            </form>

            <div className="p-4 border-t border-slate-200 bg-white flex items-center justify-between gap-3">
              {editingEvent ? <button type="button" onClick={() => handleDelete(editingEvent.id)} className="px-4 py-2.5 rounded-xl bg-red-50 text-red-700 border border-red-200 font-bold text-xs">Excluir evento</button> : <div />}
              <div className="flex gap-2">
                <button type="button" onClick={() => setModalOpen(false)} className="px-4 py-2.5 rounded-xl bg-slate-100 text-slate-700 font-bold text-xs">Cancelar</button>
                <button type="submit" form="agenda-event-form" className="px-5 py-2.5 rounded-xl bg-[#0A1F44] text-white font-bold text-xs">{editingEvent ? 'Salvar alterações' : 'Salvar na Agenda'}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {deletingEventId && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <div className="bg-white w-full max-w-sm rounded-3xl p-6 border border-slate-200 shadow-2xl space-y-4 text-center">
            <div className="w-14 h-14 rounded-2xl bg-red-100 text-red-600 flex items-center justify-center mx-auto"><span className="material-symbols-outlined text-3xl">delete_forever</span></div>
            <div><h3 className="font-extrabold text-slate-900 text-lg">Excluir compromisso</h3><p className="text-xs text-slate-500 mt-1">Confirma a exclusão deste evento da Agenda?</p></div>
            <div className="flex gap-3">
              <button type="button" onClick={() => setDeletingEventId(null)} className="flex-1 py-2.5 rounded-xl bg-slate-100 text-slate-700 font-bold text-xs">Cancelar</button>
              <button type="button" onClick={async () => {
                const target = events.find((event) => event.id === deletingEventId);
                if (target?.googleEventId && googleAccessToken) {
                  try { await deleteScheduledEventFromGoogle(target.googleEventId, googleAccessToken); }
                  catch (error) { console.error('Erro ao excluir no Google Agenda:', error); }
                }
                onDeleteEvent?.(deletingEventId);
                if (editingEvent?.id === deletingEventId) setModalOpen(false);
                setDeletingEventId(null);
              }} className="flex-1 py-2.5 rounded-xl bg-red-600 text-white font-bold text-xs">Excluir</button>
            </div>
          </div>
        </div>
      )}
