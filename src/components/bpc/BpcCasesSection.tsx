      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex flex-col md:flex-row gap-3 items-center justify-between">
        <div className="flex items-center gap-1.5 p-1 bg-slate-100 rounded-xl w-full md:w-auto">
          ...
          >
            ...
            <span>BPC Idoso ({cases.filter((c) => c.modality === 'idoso').length})</span>
          </button>
          <button
          ...
          >
            ...
            <span>BPC PCD ({cases.filter((c) => c.modality === 'pcd').length})</span>
          </button>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
        {filteredCases.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <h3 className="font-bold text-slate-900 text-sm">Nenhum caso BPC encontrado</h3>
            <p className="text-slate-500 text-xs mt-1 max-w-sm mx-auto">
              Nenhum requerimento corresponde aos filtros selecionados. Comece criando o primeiro caso do módulo.
            </p>
            <button
              onClick={onOpenNewCase}
