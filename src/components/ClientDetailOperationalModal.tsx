...
export const ClientDetailOperationalModal: React.FC<Props> = ({
  ...
  const Field = ({ label, field, type = 'text', placeholder = '' }: { label: string; field: keyof Client; type?: string; placeholder?: string }) => (
    <div>
  ...
  const SelectField = ({ label, field, options }: { label: string; field: keyof Client; options: Array<{ value: string; label: string }> }) => (
    <div>
      <label className="block font-bold text-slate-700 mb-1">{label}</label>
      {isEditing ? (
        <select
