...
export default function App() {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        try {
          ...
          if (persistedProfile) setUserProfile(persistedProfile);
        } catch (err) {
  ...
  const openCaseFromAgenda = (caseId: string) => {
    const target = cases.find((legalCase) => legalCase.id === caseId);
    setSearchQuery(target?.caseNumber || target?.processNumber || target?.clientName || '');
    setCurrentTab('cases' as NavigationTab);
  };

  const openClientFromAgenda = (clientId: string) => {
    const target = clients.find((client) => client.id === clientId);
    setSearchQuery(target?.cpf || target?.name || '');
  ...
  useEffect(() => {
    ...
    unsubWorkflows = subscribeToWorkflowTemplates(firmId, (remoteWorkflows) => {
      if (isMounted && remoteWorkflows && remoteWorkflows.length > 0) setWorkflows(remoteWorkflows);
    });
  ...
  const openDefaultTenantTemplate = () => {
    ...
    setSelectedClientForDoc(null);
    if (!tenantTemplate) {
      console.warn('[Documents] No tenant template available. Redirecting to Documents.');
  ...
  const handleSaveTemplate = (tpl: DocumentTemplate) => {
    ...
    if (exists) setTemplates(templates.map((t) => (t.id === tpl.id ? tpl : t)));
  ...
  const handleAddCategory = (cat: string) => {
    if (!docCategories.includes(cat)) setDocCategories([...docCategories, cat]);
  };
  ...
  const handleEditCategory = (oldCat: string, newCat: string) => {
    ...
    if (!userProfile?.firmId) return console.error('[Firestore Write Error] Cannot save templates for updated category: userProfile.firmId is not defined.');
    const firmId = userProfile.firmId;
    updated.filter((t) => t.category === newCat).forEach((t) => saveTemplateInFirestore(t, firmId));
  };

  const handleDeleteCategory = (cat: string) => setDocCategories(docCategories.filter((c) => c !== cat));
  const handleAddFormat = (fmt: string) => { if (!docFormats.includes(fmt)) setDocFormats([...docFormats, fmt]); };
  ...
  const handleEditFormat = (oldFmt: string, newFmt: string) => {
    ...
    setTemplates(updated);
    if (!userProfile?.firmId) return console.error('[Firestore Write Error] Cannot save templates for updated format: userProfile.firmId is not defined.');
    const firmId = userProfile.firmId;
    updated.filter((t) => t.format === newFmt).forEach((t) => saveTemplateInFirestore(t, firmId));
  };

  const handleDeleteFormat = (fmt: string) => setDocFormats(docFormats.filter((f) => f !== fmt));

  const handleClientCreated = (newClient: Client) => {
    setClients((current) => [newClient, ...current]);
    setCurrentTab('clients');
    if (!userProfile?.firmId) return console.error('[Firestore Write Error] Cannot save new client: userProfile.firmId is not defined.');
  ...
  const handleSaveClient = (updatedClient: Client) => {
    ...
    if (!userProfile?.firmId) return console.error('[Firestore Write Error] Cannot save client: userProfile.firmId is not defined.');
    saveClientInFirestore(updatedClient, userProfile.firmId);
  };

  const handleAddEvent = (newEv: ScheduledEvent) => {
    setEvents((current) => [newEv, ...current.filter((event) => event.id !== newEv.id)]);
    if (!userProfile?.firmId) return console.error('[Firestore Write Error] Cannot save new event: userProfile.firmId is not defined.');
    saveEventInFirestore(newEv, userProfile.firmId);
  };

  const handleUpdateEvent = (updatedEv: ScheduledEvent) => {
    setEvents((current) => current.map((event) => event.id === updatedEv.id ? updatedEv : event));
    if (!userProfile?.firmId) return console.error('[Firestore Write Error] Cannot save updated event: userProfile.firmId is not defined.');
  ...
  const handleDeleteEvent = (eventId: string) => {
    ...
    if (target?.caseId) {
      window.alert('Este compromisso pertence ao histórico de um caso e não pode ser excluído fisicamente. Conclua, remarque ou altere o status.');
      return;
    }
  ...
  const handleOpenDocModalForTemplate = (template: DocumentTemplate) => {
    ...
    const eligibleSelectedClient = selectedClientForDoc ? documentEligibleClients.find((client) => client.id === selectedClientForDoc.id) : null;
  ...
  const handleConfirmGenerateFromClientSelector = (generatedText: string, client: Client, template: DocumentTemplate) => {
    if (String(client.status) === 'Arquivado') return console.warn('[Documents] Archived clients cannot start document generation.');
  ...
  const handleUpdateClientField = (clientId: string, fieldKey: keyof Client, value: any) => {
    setClients((prev) => {
      const updated = prev.map((client) => client.id === clientId ? { ...client, [fieldKey]: value } : client);
      const target = updated.find((client) => client.id === clientId);
      if (target) {
        if (!userProfile?.firmId) console.error('[Firestore Write Error] Cannot update client field: userProfile.firmId is not defined.');
        else saveClientInFirestore(target, userProfile.firmId);
      }
      return updated;
    });
  };
  ...
  const handleSelectClientForDoc = (client: Client) => {
    if (String(client.status) === 'Arquivado') return console.warn('[Documents] Archived clients cannot be selected for document generation.');
    setSelectedClientForDoc(client);
    setDocClientName(client.name);
    setDocClientCpf(client.cpf);
    setSelectedTemplateForDoc(null);
  ...
  if (authLoading) {
    return <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4"><div className="w-10 h-10 border-4 border-white/20 border-t-white rounded-full animate-spin mb-4" /><p className="text-white text-xs font-bold uppercase tracking-widest">Carregando Sistema Jurídico...</p></div>;
  }
    ...
    return (
      <div className="min-h-screen bg-[#0D0D0D] flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-2xl p-8 border border-slate-200 shadow-xl text-center space-y-6">
          <div className="w-12 h-12 rounded-full bg-amber-100 text-amber-700 mx-auto flex items-center justify-center"><span className="material-symbols-outlined text-2xl">lock_person</span></div>
          <div className="space-y-2"><h2 className="text-lg font-extrabold text-slate-900">Acesso Não Provisionado</h2><p className="text-xs text-slate-600 leading-relaxed">Seu usuário foi autenticado, mas ainda não possui acesso a um escritório no AdvoDesk. Solicite ao administrador que cadastre ou autorize seu perfil.</p></div>
          <button type="button" onClick={handleLogout} className="w-full py-2.5 px-4 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold transition-all flex items-center justify-center space-x-1.5"><span className="material-symbols-outlined text-base">logout</span><span>Sair</span></button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] text-[#0D0D0D] font-body-md text-sm relative overflow-x-hidden">
      ...
      <Suspense fallback={<main className="md:ml-64 pt-24 text-center text-sm text-slate-500">Carregando...</main>}>
        ...
        {currentTab === 'documents' && <DocumentsView templates={templates} searchQuery={searchQuery} docCategories={docCategories} docFormats={docFormats} onSelectTemplateToGenerate={handleOpenDocModalForTemplate} onSaveTemplate={handleSaveTemplate} onDeleteTemplate={handleDeleteTemplate} onAddCategory={handleAddCategory} onEditCategory={handleEditCategory} onDeleteCategory={handleDeleteCategory} onAddFormat={handleAddFormat} onEditFormat={handleEditFormat} onDeleteFormat={handleDeleteFormat} />}
        {currentTab === 'calendar' && <CalendarView events={events} cases={cases} clients={clients} searchQuery={searchQuery} onAddEvent={handleAddEvent} onUpdateEvent={handleUpdateEvent} onDeleteEvent={handleDeleteEvent} onOpenCase={openCaseFromAgenda} onOpenClient={openClientFromAgenda} />}
        {currentTab === 'settings' && <SettingsView settings={{ ...settings, workflows }} onUpdateSettings={handleSaveSettings} />}
        {currentTab === 'support' && (
          <main className="md:ml-64 pt-16 md:pt-8 pb-12 px-4 sm:px-6 md:px-8 min-h-screen">
            <div className="max-w-3xl mx-auto glass-panel rounded-[24px] p-8 border border-slate-200/90 shadow-sm space-y-6">
              ...
              <p className="text-sm text-slate-600">Caso tenha dúvidas sobre o sistema ou para suporte no desenvolvimento, entre em contato através dos canais abaixo:</p>
              <div className="space-y-4 text-xs">
                <div className="p-4 rounded-xl bg-emerald-50/60 border border-emerald-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full bg-emerald-600 text-white flex items-center justify-center shrink-0 shadow-xs"><span className="material-symbols-outlined text-xl">chat</span></div><div><span className="font-bold text-slate-800 text-sm block">WhatsApp do Desenvolvedor do Sistema</span><span className="text-slate-500 text-xs">Contato direto via WhatsApp para suporte técnico</span></div></div><a href="https://wa.me/55991503232" target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-xs flex items-center gap-2 transition-all shadow-xs"><span className="material-symbols-outlined text-sm">chat</span><span>(55) 99150-3232</span></a></div>
                <div className="p-4 rounded-xl bg-blue-50/60 border border-blue-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full bg-[#0A1F44] text-white flex items-center justify-center shrink-0 shadow-xs"><span className="material-symbols-outlined text-xl">mail</span></div><div><span className="font-bold text-slate-800 text-sm block">E-mail de Suporte Técnico</span><span className="text-slate-500 text-xs">Canal oficial para dúvidas, melhorias e chamados</span></div></div><a href="mailto:codex.martis.dev@gmail.com" className="px-4 py-2 bg-[#0A1F44] hover:bg-slate-900 text-white font-bold rounded-xl text-xs flex items-center gap-2 transition-all shadow-xs"><span className="material-symbols-outlined text-sm">mail</span><span>codex.martis.dev@gmail.com</span></a></div>
              </div>
            </div>
          </main>
        )}
      ...
      <Suspense fallback={null}>
        {newCaseModalOpen && <NewCaseModal isOpen={newCaseModalOpen} onClose={() => setNewCaseModalOpen(false)} clients={clients} onClientCreated={handleClientCreated} clientCategories={settings.clientCategories} />}
        {clientSelectorModalOpen && <ClientSelectorDocumentModal isOpen={clientSelectorModalOpen} onClose={() => setClientSelectorModalOpen(false)} template={selectedTemplateForDoc} clients={documentEligibleClients} settings={settings} onConfirmGenerate={handleConfirmGenerateFromClientSelector} onUpdateClientField={handleUpdateClientField} />}
        {variablesGuideModalOpen && <VariablesGuideModal isOpen={variablesGuideModalOpen} onClose={() => setVariablesGuideModalOpen(false)} />}
        {docModalOpen && <DocumentGeneratorModal isOpen={docModalOpen} onClose={() => { setDocModalOpen(false); setSelectedClientForDoc(null); setDocClientName(''); setDocClientCpf(''); setPrefilledGeneratedText(null); }} template={selectedTemplateForDoc} clients={documentEligibleClients} initialClientName={docClientName} initialClientCpf={docClientCpf} initialGeneratedText={prefilledGeneratedText || undefined} settings={settings} />}
      </Suspense>
    </div>
  );
}
